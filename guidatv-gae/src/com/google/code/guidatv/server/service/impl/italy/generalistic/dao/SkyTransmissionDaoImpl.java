...
public class SkyTransmissionDaoImpl implements SkyTransmissionDao {
    ...
    public List<Transmission> getTransmissions(Channel channel, Date day) {
        ...
        String urlString = format.format(new Object[] { channel.getCode(),
                yearFormat.format(day), cal.get(Calendar.MONTH) + 1,
                ...
                if (id > 0 && sameDay) {
                    ...
                    if (currentTime >= lastTime) {
                        ...
                        transmissions.add(new Transmission(transmissionJson
                                .getString("title"), transmissionJson
                                .getString("desc"), cal.getTime(), null,
                                "http://guidatv.sky.it/guidatv/programma/"
                                        + transmissionJson.getString("genre").replaceAll(" ", "")
                                        + "/"
                                        + transmissionJson.getString("subgenre").replaceAll(" ", "")
                                        + "/"
                                        + transmissionJson
                                                .getString("normalizedtitle") + "_"
                                        + transmissionJson.getString("pid")
                                        + ".shtml?eventId=" + id));
                    } else {
                        sameDay = false;
                    }
