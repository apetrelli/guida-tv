...
public class DNERealTimeTransmissionDaoImpl implements DNERealTimeTransmissionDao {
    ...
    private void loadTransmissions(String type, Date day, Date urlDay,
            MessageFormat format, List<Transmission> transmissions) {
        Calendar cal = Calendar.getInstance(TimeZone.getTimeZone("Europe/Rome"));
        cal.setTime(urlDay);
        String urlString = format.format(new Object[] { type,
        ...
        try {
            ...
            for (int i=0; i < jsonArray.length() && sameDay; i++) {
                ...
                transmissions.add(new Transmission(transmissionJson
                        .getString("name"), transmissionJson
                        .getString("description"), cal.getTime(), null, null));
            }
